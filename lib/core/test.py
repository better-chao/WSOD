...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    return {'scores': scores, 'boxes' : boxes, 'blob_conv': blob_conv}
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    # print(inputs['data'][0].shape, inputs['rois'][0].shape, inputs['labels'][0].shape)
    # exit(0)
    ...
    # if cfg.SLV.SWITCH:
    #     assert len(return_dict['refine_score']) == (cfg.REFINE_TIMES + 1)
    #     scores += return_dict['refine_score'][-1].data.cpu().numpy().squeeze().reshape([-1, scores.shape[-1]])
    #     scores /= 2.
    ...
    if cfg.DEDUP_BOXES > 0:
        ...
        return_dict['blob_conv'] = return_dict['blob_conv'][inv_index, :, :]

    return scores, pred_boxes, im_scale, return_dict['blob_conv']


def im_SLV_heatmap(model, im, target_scale, target_max_size, boxes, labels):
    """Prepare the bbox for testing"""
    original_shape = im.shape[0:2]

    inputs, im_scale = _get_blobs(im, boxes, target_scale, target_max_size)

    scale_shape = inputs["data"][0].shape[-2:]
    ...
    def Output_M(proposal_sorces, rois_, M):
        for temp_inds in range(rois_.shape[0]):
            ...
            M_scores[np.where(M_scores < 0.2)] = 0
            add_score = np.zeros_like(target_field) + np.reshape(M_scores, [1, 1, -1])
            # add_score = np.maximum(add_score, target_field)
    ...
    # print(original_shape, scale_shape, labels.shape, pred_boxes.shape, scores.shape, labels)

    valid_class = np.where(labels[0] == 1)[0]
    ...
    valid_refine_scores = scores[:, valid_class]
        
    numpy_M = Output_M(valid_refine_scores, pred_boxes, M)
    # print(np.min(numpy_M), np.max(numpy_M))
...
def im_detect_bbox_aug(model, im, box_proposals=None):
    ...
    def add_preds_t(scores_t, boxes_t, blob_t):
        # print(scores_t.shape, boxes_t.shape, blob_t.shape)
        scores_ts.append(scores_t)
        boxes_ts.append(boxes_t)
        # print(blob_t.shape)
        if isinstance(blob_t, torch.Tensor):
            blob_t = blob_t.cpu().numpy()
        blob_ts.append(blob_t)
    ...
    if cfg.TEST.BBOX_AUG.H_FLIP:
        ...
        add_preds_t(scores_hf, boxes_hf, _)

    # Compute detections at different scales
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        max_size = cfg.TEST.BBOX_AUG.MAX_SIZE
        scores_scl, boxes_scl, _ = im_detect_bbox_scale(
            model, im, scale, max_size, box_proposals
        )
        add_preds_t(scores_scl, boxes_scl, _)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            scores_scl_hf, boxes_scl_hf, _ = im_detect_bbox_scale(
                model, im, scale, max_size, box_proposals, hflip=True
            )
            add_preds_t(scores_scl_hf, boxes_scl_hf, _)

    # Perform detection at different aspect ratios
    for aspect_ratio in cfg.TEST.BBOX_AUG.ASPECT_RATIOS:
        scores_ar, boxes_ar, _ = im_detect_bbox_aspect_ratio(
            model, im, aspect_ratio, box_proposals
        )
        add_preds_t(scores_ar, boxes_ar, _)

        if cfg.TEST.BBOX_AUG.ASPECT_RATIO_H_FLIP:
            scores_ar_hf, boxes_ar_hf, _ = im_detect_bbox_aspect_ratio(
                model, im, aspect_ratio, box_proposals, hflip=True
            )
            add_preds_t(scores_ar_hf, boxes_ar_hf, _)
    ...
    add_preds_t(scores_i, boxes_i, blob_conv_i)
    ...
    return scores_c, boxes_c, im_scale_i, blob_c
...
def im_detect_bbox_hflip(
    ...
    return scores_hf, boxes_inv, _[:, :, ::-1]
...
def box_results_with_nms_and_limit(scores, boxes):  # NOTE: support single-batch
    ...
    num_classes = cfg.MODEL.NUM_CLASSES + 1
    cls_boxes = [[] for _ in range(num_classes)]
    keep_inds = [[] for _ in range(num_classes)]
    # Apply threshold on detection probabilities and apply NMS
    # Skip j = 0, because it's the background class
    for j in range(1, num_classes):
    ...
    if cfg.TEST.DETECTIONS_PER_IM > 0:
        ...
        if len(image_scores) > cfg.TEST.DETECTIONS_PER_IM:
            ...
            for j in range(1, num_classes):
                ...
                keep_inds[j] = keep_inds[j][keep]

    im_results = np.vstack([cls_boxes[j] for j in range(1, num_classes)])
    boxes = im_results[:, :-1]
    scores = im_results[:, -1]
    # return scores, boxes, cls_boxes
    return scores, keep_inds, cls_boxes
