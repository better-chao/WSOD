...
from modeling.dropblock import DropBlock, ConvConcreteDB
from modeling.ADL import ADL
...
class dilated_conv5_body(nn.Module):
    ...
    def train(self, mode=True):
        ...
        if cfg.ADL.SWITCH:
            getattr(self, 'ADL3').train(mode)
            getattr(self, 'ADL4').train(mode)
            getattr(self, 'ADL5').train(mode)

    def forward(self, x):
        adl_x = None

        if not cfg.ADL.SWITCH:
            for i in range(1, 6):
                    x = getattr(self, 'conv%d' % i)(x)
        else:
            x = getattr(self, 'conv1')(x)
            x = getattr(self, 'conv2')(x)
            x = getattr(self, 'conv3')(x)
            adl_x = getattr(self, 'ADL3')(x)
            x = getattr(self, 'conv4')(x)
            adl_x = getattr(self, 'conv4')(adl_x)
            adl_x = getattr(self, 'ADL4')(adl_x)
            x = getattr(self, 'conv5')(x)
            adl_x = getattr(self, 'conv5')(adl_x)
            adl_x = getattr(self, 'ADL5')(adl_x)
        return x, adl_x


class roi_2mlp_head(nn.Module):
    def __init__(self, dim_in, roi_xform_func, spatial_scale, cfg):
        super().__init__()
        self.dim_in = dim_in
