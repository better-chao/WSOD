...
class JsonDataset(object):
    ...
    def _add_gt_annotations(self, entry):
        ...
        for ix, obj in enumerate(valid_objs):
            ...
            gt_boxes = np.vstack((gt_boxes, np.asarray(obj['clean_bbox']).reshape(1, -1)))
            gt_classes[ix] = cls
            if "weight" in obj:
                entry["history_label"].append([obj["weight"]])
            else:
                entry["history_label"].append([1])
        
        entry['gt_boxes_classes'] = np.expand_dims(gt_classes, 0)
    ...
    def _add_proposals_from_file(
        ...
        for i, entry in enumerate(roidb):
            ...
            # print(entry['id'], proposals[id_field][i])
