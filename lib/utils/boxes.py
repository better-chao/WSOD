...
def intersect_np(box_a, box_b):
    ...
    A = box_a.shape[0]
    B = box_b.shape[0]
    max_xy = np.minimum(np.expand_dims(box_a[:, 2:], axis=1),
                       np.expand_dims(box_b[:, 2:], axis=0))
    min_xy = np.maximum(np.expand_dims(box_a[:, :2], axis=1),
                       np.expand_dims(box_b[:, :2], axis=0))
    inter = np.maximum((max_xy - min_xy), 0)
    return inter[:, :, 0] * inter[:, :, 1]


def bbox_contain(box_a, box_b):
    """Compute the jaccard overlap of two sets of boxes.  The jaccard overlap
    is simply the intersection over union of two boxes.  Here we operate on
    ground truth boxes and default boxes.
    E.g.:
    ...
    area_a = np.expand_dims((box_a[:, 2]-box_a[:, 0]) *
              (box_a[:, 3]-box_a[:, 1]), axis=1)  # [A,B]
    area_b = np.expand_dims((box_b[:, 2]-box_b[:, 0]) *
              (box_b[:, 3]-box_b[:, 1]), axis=0)  # [A,B]
    union = area_a + area_b - inter
