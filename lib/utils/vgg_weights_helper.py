...
def load_pretrained_imagenet_weights(model):
    ...
    if ext == '.pkl':
        ...
        if cfg.VGG.IMAGENET_PRETRAINED_WEIGHTS.split("/")[0] == "Outputs":
            pretrianed_state_dict = convert_state_dict_from_adl(torch.load(weights_file)["model"])
        else:
    ...
    for k, v in name_mapping.items():
        if isinstance(v, str):  # maybe a str, None or True
            if pattern.match(v):
                ...
                if cfg.VGG.IMAGENET_PRETRAINED_WEIGHTS.split("/")[0] == "Outputs" and "fc" in pretrianed_key: continue
                if ext == '.pkl':
...
def convert_state_dict_from_adl(src_dict):
    """Return the correct mapping of tensor name and value

    Mapping from the names of torchvision model to our vgg conv_body and box_head.
    """
    dst_dict = {}
    dst_dict['conv1.0.weight'] = src_dict['Conv_Body.conv1.0.weight']
    dst_dict['conv1.0.bias'] = src_dict['Conv_Body.conv1.0.bias']
    dst_dict['conv1.2.weight'] = src_dict['Conv_Body.conv1.2.weight']
    dst_dict['conv1.2.bias'] = src_dict['Conv_Body.conv1.2.bias']
    dst_dict['conv2.0.weight'] = src_dict['Conv_Body.conv2.0.weight']
    dst_dict['conv2.0.bias'] = src_dict['Conv_Body.conv2.0.bias']
    dst_dict['conv2.2.weight'] = src_dict['Conv_Body.conv2.2.weight']
    dst_dict['conv2.2.bias'] = src_dict['Conv_Body.conv2.2.bias']
    dst_dict['conv3.0.weight'] = src_dict['Conv_Body.conv3.0.weight']
    dst_dict['conv3.0.bias'] = src_dict['Conv_Body.conv3.0.bias']
    dst_dict['conv3.2.weight'] = src_dict['Conv_Body.conv3.2.weight']
    dst_dict['conv3.2.bias'] = src_dict['Conv_Body.conv3.2.bias']
    dst_dict['conv3.4.weight'] = src_dict['Conv_Body.conv3.4.weight']
    dst_dict['conv3.4.bias'] = src_dict['Conv_Body.conv3.4.bias']
    dst_dict['conv4.0.weight'] = src_dict['Conv_Body.conv4.0.weight']
    dst_dict['conv4.0.bias'] = src_dict['Conv_Body.conv4.0.bias']
    dst_dict['conv4.2.weight'] = src_dict['Conv_Body.conv4.2.weight']
    dst_dict['conv4.2.bias'] = src_dict['Conv_Body.conv4.2.bias']
    dst_dict['conv4.4.weight'] = src_dict['Conv_Body.conv4.4.weight']
    dst_dict['conv4.4.bias'] = src_dict['Conv_Body.conv4.4.bias']
    dst_dict['conv5.0.weight'] = src_dict['Conv_Body.conv5.0.weight']
    dst_dict['conv5.0.bias'] = src_dict['Conv_Body.conv5.0.bias']
    dst_dict['conv5.2.weight'] = src_dict['Conv_Body.conv5.2.weight']
    dst_dict['conv5.2.bias'] = src_dict['Conv_Body.conv5.2.bias']
    dst_dict['conv5.4.weight'] = src_dict['Conv_Body.conv5.4.weight']
    dst_dict['conv5.4.bias'] = src_dict['Conv_Body.conv5.4.bias']
    return dst_dict
