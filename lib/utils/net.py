...
def smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, cls_loss_weights, beta=1.0):
    ...
    loss_box_for_train = loss_box.view(-1).sum(0) / (N + 1e-6)
    return loss_box_for_train


def smooth_l1_loss_2(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, cls_loss_weights, beta=1.0, gt_assignment=None, gt_box_num=None):
    """
    SmoothL1(x) = 0.5 * x^2 / beta      if |x| < beta
                  |x| - 0.5 * beta      otherwise.
    1 / N * sum_i alpha_out[i] * SmoothL1(alpha_in[i] * (y_hat[i] - y[i])).
    ...
    box_diff = bbox_pred - bbox_targets
    in_box_diff = bbox_inside_weights * box_diff
    # in_box_diff_no_reweight = box_diff

    abs_in_box_diff = torch.abs(in_box_diff)
    # abs_in_box_diff_no_reweight = torch.abs(in_box_diff_no_reweight)
    ...
    for box_th_for_record in range(gt_box_num):
        append_loss = in_loss_box[gt_assignment == box_th_for_record]
        if append_loss.shape[0] > 0:
            reg_loss_per_anno.append(append_loss.sum(-1).mean().item())
        else:
            reg_loss_per_anno.append(10.0)

    out_loss_box = bbox_outside_weights * in_loss_box
    loss_box = out_loss_box
    N = (cls_loss_weights > 0).sum()
    loss_box_for_train = loss_box.view(-1).sum(0) / (N + 1e-6)
    return loss_box_for_train, reg_loss_per_anno
