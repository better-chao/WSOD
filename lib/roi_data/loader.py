...
class MixUpBatchSampler(torch_sampler.BatchSampler):
    def __init__(self, sampler, batch_size, drop_last):
        if not isinstance(sampler, torch_sampler.Sampler):
            raise ValueError("sampler should be an instance of "
                             "torch.utils.data.Sampler, but got sampler={}"
...
def collate_minibatch(list_of_blobs):
    ...
    for blobs in list_of_blobs:
        lists.append({'data' : blobs.pop('data'),
                      ...
                      'gt_boxes': blobs.pop('gt_boxes'),
                      'gt_boxes_classes': blobs.pop('gt_boxes_classes'),
                      'history_label': blobs.pop('history_label'),
                      'image_name': blobs.pop('image_name')
                      })
